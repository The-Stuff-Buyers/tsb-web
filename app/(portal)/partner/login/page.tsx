...
function LoginForm() {
  ...
  const partnerName = searchParams.get('partner') || 'bidfta'
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/partner/login', {
        ...
        body: JSON.stringify({ partner_name: partnerName, password }),
      })
      const data = await res.json()
      if (!res.ok) { setError(data.error || 'Invalid credentials.'); setState('error'); return }
      window.location.href = `/partner/${partnerName}`
    } catch {
  ...
  const displayName = partnerName === 'bidfta' ? 'BidFTA' : partnerName
      ...
      <div className="page">
        <div className="card">
          <div className="card-header">
            ...
            <div className="tagline">{displayName} Partner Portal</div>
          </div>
          <form onSubmit={handleSubmit} className="login-form">
            ...
            <button type="submit" className="submit-btn" disabled={state === 'loading'}>
              {state === 'loading' ? '◆  LOGGING IN…' : '◆  ACCESS PORTAL'}
            </button>
          </form>
          <div className="card-footer">Questions? quotes@thestuffbuyers.com</div>
        </div>
      </div>
    </>
  )
